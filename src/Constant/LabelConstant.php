<?php
namespace src\Constant;

interface LabelConstant
{
    public const LBL_HOME               = 'Accueil';
    public const LBL_ACRONYMS           = 'Acronymes';
    public const LBL_ADDRESS            = 'Adresse';
    public const LBL_AGE                = 'Age';
    public const LBL_GAME_AIDS          = 'Aides de jeu';
    public const LBL_NO_RESULTS         = 'Aucun résultat';
    public const LBL_OTHERS             = 'Autres';
    public const LBL_DATABASE           = 'Base de données';
    public const LBL_LIBRARY            = 'Bibliothèque';
    public const LBL_DESK               = 'Bureau';
    public const LBL_CATEGORY           = 'Catégorie';
    public const LBL_CODE               = 'Code';
    public const LBL_ZIPCODE            = 'Code Postal';
    public const LBL_SKILLS             = 'Compétences';
    public const LBL_FLD_COMPTE         = 'Compte - dépenses - niveau de vie';
    public const LBL_LOGIN              = 'Connexion';
    public const LBL_COPS               = 'COPS';
    public const LBL_TRASH              = 'Corbeille';
    public const LBL_DATE               = 'Date';
    public const LBL_DATE_HEURE         = 'Date / Heure';
    public const LBL_DESCRIPTION        = 'Description';
    public const LBL_DISTRICT_ATTORNEY  = 'District Attorney';
    public const LBL_ECHELON            = 'Echelon';
    public const LBL_ENQUETES           = 'Enquêtes';
    public const LBL_EVENT              = 'Evénement';
    public const LBL_FACTS              = 'Faits';
    public const LBL_END_SHIFT_DATE     = 'Fin de service';
    public const LBL_FLD_FREQUENTATION  = 'Fréquentations - collègues - famille - amis';
    public const LBL_GENDER             = 'Genre';
    public const LBL_GRADE              = 'Grade';
    public const LBL_GROUP              = 'Groupe';
    public const LBL_FLD_HABITATION     = 'Habitations - quartiers - voitures';
    public const LBL_HOUR               = 'Heure';
    public const LBL_ID                 = 'Identifiant';
    public const LBL_CLUES              = 'Indices';
    public const LBL_INTEGRATION_DATE   = 'Intégration';
    public const LBL_FLD_LOISIR         = 'Loisirs - habitudes - emploi';
    public const LBL_MATRICULE          = 'Matricule';
    public const LBL_MDP                = 'Mot de passe';
    public const LBL_BIRTHDAY           = 'Naissance';
    public const LBL_NAMESET            = 'Nameset';
    public const LBL_LEVEL              = 'Niveau';
    public const LBL_NAME               = 'Nom';
    public const LBL_NOTIFICATIONS      = 'Notifications';
    public const LBL_OCCUPATION         = 'Occupation';
    public const LBL_CODE_OPTIMISATION  = 'Optimisation code';
    public const LBL_SETTINGS           = 'Paramètres';
    public const LBL_BINOME             = 'Partenaire';
    public const LBL_PISTES_DEMARCHES   = 'Pistes / Démarches';
    public const LBL_WEIGHT             = 'Poids';
    public const LBL_FIRST_DETECTIVE    = 'Premier détective';
    public const LBL_FIRSTNAME          = 'Prénom';
    public const LBL_PROFILE            = 'Profil';
    public const LBL_GAL_RESUME         = 'Résumé des faits';
    public const LBL_CENTRAL_LAPD       = 'Quartier Général LAPD';
    public const LBL_SECTION            = 'Section';
    public const LBL_SUBMIT             = 'Soumettre';
    public const LBL_SOURCE             = 'Source';
    public const LBL_COURSES            = 'Stages';
    public const LBL_SURNAME            = 'Surnom';
    public const LBL_HEIGHT             = 'Taille';
    public const LBL_FLD_TELEPHONE      = 'Téléphone - ordinateur - agenda';
    public const LBL_FLD_TEMOIGNAGE     = 'Témoignages';
    public const LBL_CITY               = 'Ville';

    public const LBL_ERR_LOGIN          = 'Une erreur est survenue lors de la saisie de votre identifiant et de votre mot de passe.<br>L\'un des champs était vide, ou les deux ne correspondaient pas à une valeur attendue.<br>Veuillez réessayer ou contacter un administrateur.<br><br>';
    public const LBL_ERR_MDPCHG         = 'Une erreur est survenue lors du changement de votre mot de passe.<br>Soit vous n\'avez pas saisi correctement votre ancien mot de passe, soit le nouveau mot de passe et sa confirmation ne correspondent pas.<br><br>';
    
}
